...
func Rename(path, newname string) service.FileStatus {
	...
	if dbGet(formatKey(parentid, ana[len(ana)-1])) == "" {
		return service.FileStatus_fPathNotFound
	}
	if dbGet(formatKey(parentid, newname)) != "" {
		return service.FileStatus_fExist
	}
	dbPut(formatKey(parentid, newname), updateModtime(dbGet(formatKey(parentid, ana[len(ana)-1]))))
	dbDelete(formatKey(parentid, ana[len(ana)-1]))
	return 0
}

func Delete(path string) service.FileStatus {
	ana := strings.Split(path, "/")
	if ana[len(ana)-1] == "" {
		ana = ana[:len(ana)-1]
	}
...
func CheckPath(path, filename string) (service.FileStatus, int64) {
	ana := strings.Split(path, "/")
	if ana[len(ana)-1] == "" {
		ana = ana[:len(ana)-1]
	}
